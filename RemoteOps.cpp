...
inline void strlwrcpy(char* dst, const char* src)
{
	if (strlen(src) > 0)
...
HMODULE WINAPI GetRemoteModuleHandle(HANDLE hProcess, LPCSTR lpModuleName)
{
	...
	strlwrcpy(lpModuleNameCopy, lpModuleName);
	...
	for (DWORD i = 0; i <= NumModules; ++i)
	{
		...
		mbstowcs(tmp, ModuleNameBuffer, strlen(ModuleNameBuffer) + 1);
		/* Get the module's name */
		::GetModuleBaseName(hProcess, ModuleArray[i], tmp, sizeof(ModuleNameBuffer));
		_bstr_t d(tmp);

		strcpy(ModuleNameBuffer, d);

		/* Convert ModuleNameBuffer to all lowercase so the comparison isn't case sensitive */
		_strlwr(ModuleNameBuffer);

		/* Does the name match? */
		if (strcmp(ModuleNameBuffer, lpModuleNameCopy) == NULL)
		{
...
FARPROC WINAPI GetRemoteProcAddress(HANDLE hProcess, HMODULE hModule, LPCSTR lpProcName, UINT Ordinal, BOOL UseOrdinal)
{
	...
	if (UseOrdinal)
	{
			...
			ExportFunctionTable[FunctionTableIndex] <= ExportDirectory.VirtualAddress + ExportDirectory.Size)
		{
			...
			if (RealFunctionId.at(0) == '#') // Exported by ordinal
			{
				...
				RealOrdinal = atoi(RealFunctionId.c_str());
	...
	for (DWORD i = 0; i < ExportTable.NumberOfNames; ++i) {
		...
		/* Does the name match? */
		if (TempFunctionName.compare(lpProcName) != string::npos)
		{
			/* NOTE:
